...
The harness includes automated tests for the policy code.

The tests are not intended to prove that the harness is a complete security boundary.

They verify that the deterministic rules behave as expected for known allow and deny cases.
...
A successful run should complete with all tests passing.
...
├── test_protect_secrets.py
└── test_git_diff_budget.py
```

#### Secret Protection

Tests verify that protected resources and suspicious operations are denied while ordinary tool requests remain allowed.
...
- protected Codex configuration
- environment-variable dumping
- normal non-sensitive file access

#### Git Diff Budget

Tests verify that staged Git changes are measured before a commit is allowed.

The suite checks behavior such as:

- commits within the configured budget are allowed
- commits exceeding the file limit are denied
- commits exceeding the changed-line limit are denied
- staged changes are measured instead of the entire working tree
- policy failures prevent the commit from proceeding

#### Policy Dispatcher

Tests verify that the pre-tool policy correctly routes requests through the appropriate deterministic checks.

Secret protection is evaluated before tool execution.

Git diff-budget enforcement is applied when the requested operation attempts to create a Git commit.

### Tests Are Not Enough

Passing tests means the known policy behavior is working as expected.

It does **not** mean the harness cannot be bypassed.

After running the automated tests, intentionally try to break the policies through Codex.
...
If you find a bypass, add a failing test that reproduces it before fixing the policy.
...
The goal is not to trust the harness because it has tests.

The goal is to make the boundaries observable, reproducible, and continuously harder to bypass.

> **Don't just test the happy path. Try to break the boundary.**
